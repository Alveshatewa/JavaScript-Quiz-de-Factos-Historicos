...
function selecioarQuestoesAleatorias(questoes, quantidadeQuestoes) {

    // Seleciona aleatoriamente as questões

    const questoesAleatorias = questoes.sort(() => Math.random() - 0.5)

    // Retorna um novo array com 10 questoes selecionadas
...
function exibirPergunta(pergunta, index) {
    ...
    const respostaUsuario = entradaDados.question('Digite o ano: ')
...
function iniciarQuiz() {
    ...
    console.log(`Seja Bem-vindo jogador(a)! ${nomeJogador}!\n`);

    const quantidadeQuestoes = 10;
  
    const questoesSelecionadas = selecioarQuestoesAleatorias(questoes, quantidadeQuestoes); 
